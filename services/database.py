...
class DatabaseError(Exception):
    """Единая ошибка слоя данных для внешних потребителей"""
...
async def ensure_user(user_id: int, user_name: str | None) -> None:
    ...
    except(PostgresError, InterfaceError, OSError) as e:
        raise DatabaseError(f'ensure_user не смог выполнить операцию для {user_id}') from e
...
async def get_user_cities(user_id: int) -> list[str]:
    try:
        ...
        raise DatabaseError(f'get_user_cities не смог выполнить операцию для {user_id}') from e
...
async def add_city(user_id: int, city_name: str) -> bool:
    try:
        ...
        raise DatabaseError(f'add_city не смог выполнить операцию для {user_id}') from e
...
async def remove_city(user_id: int, city_name: str) -> bool:
    try:
        ...
        raise DatabaseError(f'remove_city не смог выполнить операцию для {user_id}') from e
